...
import { mediaDetailsPath } from "@/backend/metadata/tmdb";
...
function MediaPosterPlaceholder({ pulse = false }: { pulse?: boolean }) {
  return (
    <div
      className={classNames(
        "absolute inset-0 flex items-center justify-center bg-mediaCard-hoverBackground p-6",
        pulse && "animate-pulse",
      )}
    >
      <img
        src={MEDIA_POSTER_PLACEHOLDER}
        alt=""
        className="max-h-[42%] max-w-[72%] object-contain opacity-45"
        draggable={false}
      />
    </div>
  );
...
function MediaCardContent({
  ...
  const hasRealPoster =
    Boolean(media.poster) && media.poster !== MEDIA_POSTER_PLACEHOLDER;
      ...
      >
        ...
        >
          <div
            ...
            style={{
              backgroundImage:
                isIntersecting && hasRealPoster
                  ? `url(${media.poster})`
                  : undefined,
            }}
          >
            {(!isIntersecting || !hasRealPoster) && (
              <MediaPosterPlaceholder pulse={!isIntersecting} />
            )}
            {series ? (
              <div
                className={[
...
export function MediaCard(props: MediaCardProps) {
  ...
  // Card clicks always open the details page (episode picker + info).
  // Playback starts from Play / episode links on that page.
  const link = canLink ? mediaDetailsPath(props.media) : "#";
