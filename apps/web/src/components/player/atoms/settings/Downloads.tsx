...
/* Kept for when stream downloads return:
import { fetchGridData } from "@/lib/providerExtras";
import type { GridData } from "@/lib/providerExtras";
...
import { isTopWaatchPremiumSource } from "@/utils/topwaatchSources";
*/
...
export function DownloadView({ id }: { id: string }) {
  ...
  return (
    <>
      ...
      <Menu.Section>
        <div className="flex flex-col items-center text-center gap-3 mt-4 px-2 min-h-[11rem]">
          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-video-context-light/10">
            <Icon
              icon={Icons.WAND}
              className="text-2xl text-video-context-type-accent"
            />
          </div>
          <p className="text-[11px] font-semibold uppercase tracking-wide text-video-context-type-accent">
            {t("player.menus.downloads.comingSoon.badge")}
          </p>
          <Menu.Paragraph marginClass="mb-0">
            {t("player.menus.downloads.comingSoon.description")}
          </Menu.Paragraph>
          <p className="mt-auto pt-6 text-xs italic text-video-context-type-secondary">
            {t("player.menus.downloads.comingSoon.title")}
          </p>
        </div>
      </Menu.Section>
    </>
  ...
  /* Existing download menu  kept for when stream downloads ship.
    ...
    <>
      ...
      <Menu.Section>
        <div className="flex flex-col gap-3 mt-2">
          <button
            type="button"
            className={`w-full rounded-lg bg-video-context-light/10 transition-colors p-4 text-left relative group ${canOriginalDownload ? "hover:bg-video-context-light/20 cursor-pointer" : "opacity-50 cursor-not-allowed"}`}
            onClick={() =>
              canOriginalDownload && router.navigate("/download/original")
            }
            disabled={!canOriginalDownload}
          >
            <div className="flex items-center gap-3">
              <Icon
                icon={Icons.FILE_ARROW_DOWN}
                className={`text-2xl ${canOriginalDownload ? "text-video-context-type-accent" : "text-video-context-type-secondary"}`}
              />
...
export function DownloadRoutes({ id }: { id: string }) {
  return (
    <>
      <OverlayPage id={id} path="/download" width={343} height={360}>
        <Menu.CardWithScrollable>
          <DownloadView id={id} />
        </Menu.CardWithScrollable>
      </OverlayPage>
      {/* Sub-routes kept for when stream downloads return
