...
export function CaptionsView({
  ...
  const groupedCaptions = useMemo(() => {
    ...
    allCaptions.forEach((caption) => {
      // Prefer converting display/language names → ISO codes. Subtitle APIs
      // often send "Français" / "Indonesian" instead of "fr" / "id".
      const lang =
        labelToLanguageCode(caption.language || "") ||
        labelToLanguageCode(caption.display || "") ||
        (/^[a-z]{2,3}(-[a-z0-9]+)*$/i.test(caption.language || "")
          ? caption.language
          : null) ||
        "unknown";
      if (!groups[lang]) {
