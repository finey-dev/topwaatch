...
import { metaToScrapeMedia, playerStatus } from "@/stores/player/slices/source";
...
export function useSourceScraping(sourceId: string | null, routerId: string) {
  ...
  const [request, run] = useAsyncFn(async () => {
    ...
    if (result.stream) {
      setEmbedId(null);
      setCaption(null);
      // Set sourceId first so HEVC rejection can auto-resume / mark failed.
      setSourceId(sourceId);
      setSource(
        convertRunoutputToSource({ stream: result.stream[0] }),
        convertProviderCaption(result.stream[0].captions),
      ...
      // If setSource rejected the stream (e.g. HEVC-only), stay on the source
      // picker so the user can choose a different source.
      if (usePlayerStore.getState().status !== playerStatus.PLAYBACK_ERROR) {
        if (enableLastSuccessfulSource) {
          setLastSuccessfulSource(sourceId);
        }
        router.close();
