import { RunOutput, labelToLanguageCode } from "@topwaatch/providers";
...
export function convertProviderCaption(
  ...
  return captions.map((v) => {
    const language =
      labelToLanguageCode(v.language || "") ||
      labelToLanguageCode((v as any).display || "") ||
      (/^[a-z]{2,3}(-[a-z0-9]+)*$/i.test(v.language || "")
