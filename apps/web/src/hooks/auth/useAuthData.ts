...
import { SessionResponse } from "@/backend/accounts/user";
...
export function useAuthData() {
  ...
  const syncData = useCallback(
    async (
      ...
      } else if (settings.proxyUrls === null) {
        setProxySet(null);
      }
      ...
      if (settings.homeSectionOrder !== undefined) {
        ...
        setHomeSectionOrder(
          order && order.length > 0 ? order : ["watching", "bookmarks"],
        );
      }
