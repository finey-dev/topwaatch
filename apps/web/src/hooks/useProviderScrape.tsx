import { FullScraperEvents, RunOutput, ScrapeMedia } from "@topwaatch/providers";
...
import {
  runServerScrape,
  type ScrapeSseEvent,
} from "@/backend/providers/serverScrape";
import { getMediaKey } from "@/stores/player/slices/source";
import { usePlayerStore } from "@/stores/player/store";
import { browserCanPlayHevc } from "@/stores/player/utils/qualities";
import { usePreferencesStore } from "@/stores/preferences";
import { useAuthStore } from "@/stores/auth";
import { prioritizeCinemaSourceIds } from "@/utils/topwaatchSources";
  ...
  const handleSseEvent = useCallback(
    (evt: ScrapeSseEvent) => {
      if (evt.event === "init") {
        ...
        const { id, status, percentage, reason } = evt.data;
        if (status === "started") {
          startEvent(id);
        } else {
          updateEvent({
            id,
            status: (status === "not_found" ? "notfound" : status) as
              | "pending"
              | "success"
              | "failure"
              | "notfound",
      ...
      } else if (evt.event === "discoverEmbeds") {
        discoverEmbedsEvent(evt.data);
      }
    },
    [initEvent, startEvent, updateEvent, discoverEmbedsEvent],
  );
...
export function useScrape() {
  ...
  const startScraping = useCallback(
    async (media: ScrapeMedia, startFromSourceId?: string) => {
      ...
      startScrape();
      const output = await runServerScrape({
        media,
