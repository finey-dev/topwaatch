...
export function getPrettyLanguageNameFromLocale(locale: string): string | null {
  ...
  // getTag() logs loudly for non-BCP-47 strings (e.g. "Français"). Only call
  // it with something that looks like a language tag.
  const candidate = locale.trim();
  if (!/^[a-z]{2,3}(-[a-z0-9]+)*$/i.test(candidate)) {
    return null;
  }

  const tag =
    candidate.length === 3
      ? getTag(iso6393To1[candidate] ?? candidate, true)
      : getTag(candidate, true);
