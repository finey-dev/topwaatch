...
export async function scrapeRottenTomatoes(
  ...
  const hasProxy =
    Boolean(useAuthStore.getState().proxySet) || getProxyUrls().length > 0;

  if (!hasProxy) {
    throw new Error(
      "Rotten Tomatoes scraping requires a CORS proxy. Configure one in Settings → Connections.",
    );
  }

  // eslint-disable-next-line no-console
  console.log("[RT Scraper] Using CORS proxy for requests");
  ...
  // Fetch search results via CORS proxy
  const response = await proxiedFetch<string>(searchUrl, {
    headers: {
      "User-Agent":
  ...
  try {
    ...
    const movies = movieRows.map((row: string) => {
