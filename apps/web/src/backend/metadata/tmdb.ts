...
/** Details page path  card clicks land here before the player. */
export function mediaDetailsPath(media: MediaItem): string {
  return `/details/${encodeURIComponent(mediaItemToId(media))}`;
}

/** Build a details path from loose media shapes (cards, featured carousel, etc.). */
export function detailsPathFromMedia(media: {
  id?: string | number | null;
  title?: string;
  name?: string;
  type: "movie" | "show";
}): string {
  return mediaDetailsPath({
    id: String(media.id ?? ""),
...
/** Player path for a title (optional season/episode for shows). */
export function mediaPlayerPath(
  media: MediaItem,
  series?: { seasonId: string; episodeId: string },
): string {
  let path = `/media/${encodeURIComponent(mediaItemToId(media))}`;
  if (series?.seasonId && series?.episodeId) {
    path += `/${encodeURIComponent(series.seasonId)}/${encodeURIComponent(series.episodeId)}`;
  }
  return path;
}
...
function isV4Token(key: string): boolean {
  return key.trim().split(".").length === 3;
}

/**
 * Same-origin Vite/Vercel proxy path. Browser requests with Authorization: Bearer
 * to api.themoviedb.org fail CORS preflight; proxying avoids that.
 */
const tmdbSameOriginBase = "/tmdb/3/";

function getBrowserTmdbBases(apiKey: string): {
  primary: string;
  fallback: string;
...
export async function get<T>(url: string, params?: object): Promise<T> {
  ...
  const apiKey = conf().TMDB_READ_API_KEY?.trim();
  ...
  // Callers sometimes pass api_key/language; strip them so a v4 JWT is never
  // sent as ?api_key= (invalid) and language stays consistent.
  const rawParams = { ...(params as Record<string, unknown> | undefined) };
  delete rawParams.api_key;
  delete rawParams.language;
  ...
  const bases = getBrowserTmdbBases(apiKey);

  // Absolute TMDB URL for the worker proxy destination param
  const fullUrl = new URL(
    url.replace(/^\//, ""),
    bases.absoluteOrigin.endsWith("/")
      ? bases.absoluteOrigin
      : `${bases.absoluteOrigin}/`,
  );
  ...
  async function fetchDirect(baseURL: string, timeout: number): Promise<T> {
    return mwFetch<T>(encodeURI(url), {
      headers: tmdbHeaders,
      baseURL,
      params: allParams,
      signal: abortOnTimeout(timeout),
    });
  }
  ...
  if (proxy && shouldProxyTmdb) {
    try {
      ...
      console.error(err);
      // Fall through to try direct / same-origin connection
    }
  }
  ...
  if (proxied !== undefined) {
    ...
    try {
      result = await fetchDirect(bases.primary, 5000);
    } catch {
      result = await fetchDirect(bases.fallback, 30000);
    }
  }
