...
import { ProviderMetricsPart } from "./parts/settings/ProviderMetricsPart";
...
export function AccountSettings(props: { account: AccountWithToken }) {
  return <AccountOverviewPart account={props.account} />;
}

export function SettingsPage() {
  const loc = useLocation();
  const accountGate = useAuthStore((s) => s.account);

  // Settings requires an account
  if (!accountGate) {
    return (
      <Navigate
        replace
        to={{
          pathname: "/login",
          search: `redirect=${encodeURIComponent(loc.pathname + loc.search + loc.hash)}`,
        }}
      />
    );
  }
  ...
  useEffect(() => {
    const loadSettings = async () => {
      if (account && backendUrl) {
        ...
        // Details modal removed  always keep disabled (details page is used instead)
        ...
        if (
          ...
          setHomeSectionOrder(["watching", "bookmarks"]);
        }
