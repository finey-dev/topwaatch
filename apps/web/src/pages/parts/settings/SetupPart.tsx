...
function getBackendBaseUrl(): string | null {
  const fromStore = useAuthStore.getState().backendUrl;
  const config = conf();
  return (
    fromStore ??
    config.BACKEND_URL ??
...
/** Call TopWaatch server → Febbox console traffic API. */
export async function checkFebboxKey(febboxKey: string | null): Promise<{
  status: Status;
  quota: any;
}> {
  if (!febboxKey) {
    return { status: "unset", quota: null };
  }

  const backend = getBackendBaseUrl();
  if (!backend) {
    return { status: "api_down", quota: null };
  }

  try {
    const response = await fetch(
      `${backend.replace(/\/$/, "")}/febbox/traffic?ui=${encodeURIComponent(febboxKey)}`,
    );
    ...
    const status = data.status ?? "api_down";
    return {
      status:
        status === "success" ||
        status === "unset" ||
...
export async function fetchFebboxQuota(febboxKey: string | null): Promise<any> {
  const result = await checkFebboxKey(febboxKey);
  return result.quota;
}
    ...
    <SettingsCard>
      <div className="flex flex-col md:flex-row items-start gap-4">
        ...
        <div className="md:mt-5">
          <Button
            theme="purple"
            onClick={() => navigate("/settings#settings-connection")}
          >
