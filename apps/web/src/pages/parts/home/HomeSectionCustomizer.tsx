...
export function HomeSectionCustomizer({
  ...
  const persistOrder = (newOrder: string[]) => {
    // Never persist an empty layout  fall back to defaults
    const order =
      newOrder.length > 0 ? newOrder : ["watching", "bookmarks"];
    setHomeSectionOrder(order);
    if (account && backendUrl) {
      updateSettings(backendUrl, account, { homeSectionOrder: order }).catch(
