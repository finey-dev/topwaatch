...
import { MEDIA_POSTER_PLACEHOLDER } from "@/assets/brand";
...
export function MediaCarousel({
  ...
  const contentType = React.useMemo(() => {
    if (showProviders) return selectedProviderId ? "provider" : content.type;
    if (showGenres) return selectedGenreId ? "genre" : content.type;
  ...
  const selectedId =
    selectedProviderId || selectedGenreId || selectedRecommendationId;

  // Genre/provider carousels must wait for an ID before fetching
  const fetchEnabled = !(
    (showGenres || showProviders || showRecommendations) &&
    !selectedId
  );
  ...
  const waitingForSelection = !fetchEnabled;
  const showSkeletons =
    waitingForSelection ||
    isLoading ||
    (!hasFetched && media.length === 0);
  // Only hide empty sections after a successful fetch with no results
  const shouldHide = hasFetched && !isLoading && !error && media.length === 0;
  ...
  // Hide the entire section if there's no content after loading
  if (shouldHide) {
    return null;
  }
      ...
      <div className="flex items-center justify-between ml-2 md:ml-8 mt-2">
        <div className="flex flex-col pl-2 lg:pl-[68px]">
          <div className="flex items-center gap-4">
            <h2 className="text-2xl cursor-default font-bold text-white md:text-2xl pl-0 text-balance min-h-[1.75rem]">
              {sectionTitle || (
                <span className="inline-block h-7 w-48 animate-pulse rounded bg-mediaCard-hoverBackground align-middle" />
              )}
            </h2>
            {showRecommendations &&
              recommendationSources &&
      ...
      <div className="relative overflow-hidden carousel-container md:pb-4">
        <div
          ...
          ref={(el) => {
            ...
            setCarouselEl((prev) => (prev === el ? prev : el));
          }}
        >
          <div className="lg:w-12" />

          {error && hasFetched && !isLoading ? (
            <div className="relative mt-4 flex min-h-[16rem] w-[min(100%,28rem)] flex-col items-start justify-center gap-3 px-2 text-sm text-type-secondary">
              <p>{t("discover.carousel.error", "Couldn't load this list.")}</p>
              <button
                type="button"
                onClick={() => {
                  void refetch();
                }}
                className="rounded-lg bg-mediaCard-hoverBackground px-3 py-1.5 text-type-logo transition-colors hover:text-white"
              >
                {t("discover.carousel.retry", "Try again")}
              </button>
            </div>
          ) : showSkeletons ? (
          ...
          {moreContent && generatedMoreLink && !showSkeletons && !error && (
