...
export function FeaturedCarousel({
  ...
  >({});
  const hasProxy = useRef<boolean>(false);
  ...
  // Proxy available for IMDb enrichment
  useEffect(() => {
    hasProxy.current = getProxyUrls().length > 0;
  }, []);

  // Fetch IMDb ratings when media changes
  useEffect(() => {
    const fetchImdbRatings = async () => {
      if (!hasProxy.current || !currentMedia?.external_ids?.imdb_id) return;
