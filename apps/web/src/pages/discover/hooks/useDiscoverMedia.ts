...
export function useDiscoverMedia({
  ...
  const fetchTraktMedia = useCallback(
    async (traktFunction: () => Promise<TraktListResponse>) => {
      try {
        ...
        // Fetch details for each TMDB ID with limited concurrency
        ...
        const settledResults = await mapWithConcurrency(
          ...
          async (tmdbId: number) => {
            ...
            try {
              ...
              console.error(
                `Error fetching details for TMDB ID ${tmdbId}:`,
                err,
              );
              return null;
            }
  ...
  const fetchMedia = useCallback(async () => {
    // Skip fetching until required IDs are available (genres/providers load async)
    if (
