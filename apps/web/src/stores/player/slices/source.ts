...
export const createSourceSlice: MakeSlice<SourceSlice> = (set, get) => ({
  ...
  setSource(
    ...
    if (stream.type === "file") {
      ...
      qualities = getPlayableFileQualities(stream);
    }
    const qualityPreferences = useQualityStore.getState();

    let loadableStream: ReturnType<typeof selectQuality>;
    try {
      loadableStream = selectQuality(stream, qualityPreferences.quality);
    } catch (err) {
      // selectQuality throws when the source is entirely unplayable (e.g.
      // HEVC-only on a browser that can't decode H.265). Surface this as a
      // playback error so the player UI can show the right message and the
      // auto-scraping loop can move on to the next source.
      console.warn("[player] setSource rejected:", err);
      const message = err instanceof Error ? err.message : String(err);
      set((s) => {
        s.interface.error = {
          type: "htmlvideo",
          errorName: message.includes("HEVC") ? "HevcUnsupported" : "Error",
          key: message,
        };
        s.status = playerStatus.PLAYBACK_ERROR;
        // Clear the loading spinner  display.load() was never called so
        // emit("loading", false) will never fire on its own.
  ...
  switchQuality(quality) {
    ...
    if (store.source.type === "file") {
      ...
      // Don't switch into an HEVC file the browser can't decode  that causes
      // black video with audio still playing (common on Linux Chrome).
      if (
        !browserCanPlayHevc() &&
        isLikelyHevcUrl(selectedQuality.url)
      ) {
        console.warn(
          `[player] refusing quality "${quality}": HEVC URL, browser cannot decode H.265`,
        );
        return;
      }
