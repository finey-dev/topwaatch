...
Let's get started with `@topwaatch/providers`. First lets install the package.

::alert{type="warning"}
**Note:** The `@topwaatch/providers` package is not published on npm. Install it directly from GitHub using the following commands:
::

::code-group
  ```bash [NPM]
  npm install @topwaatch/providers@github:p-stream/providers#production
  ```
  ```bash [Yarn]
  yarn add @topwaatch/providers@github:p-stream/providers#production
  ```
  ```bash [PNPM]
  pnpm add @topwaatch/providers@github:p-stream/providers#production
  ```
::
...
```ts [index.ts (server)]
import { makeProviders, makeStandardFetcher, targets } from '@topwaatch/providers';
